...
class UpcomingPlaylistClient:
    ...
    def process_gigs(
        self, gigs: list[Gig], user_details: dict[str, User] = None
    ) -> dict[str : list[str]]:
        """
            ...
            if gig.spotifyArtistId not in gigs_by_user[gig.userId]:
                ...
                try:
                    artist_added = self.spotify_client.add_artist(
                        gig.spotifyArtistId, playlist_id
                    )
                ...
                if artist_added:
                    try:
                        ...
                        logger.warning(
                            f"Unable to schedule delete for gig {gig.id}: {err}"
                        )
                        try:
    ...
    def _schedule_removal_of_artist(self, gig: Gig, playlist_id: str) -> str:
        delete_date = datetime.strftime(
            gig.date + timedelta(days=1), "%Y-%m-%dT%H:%M:%S"
        )
