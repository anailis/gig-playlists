...
class SpotifyPlaylistClient:
    ...
    def _artist_is_in_playlist(self, artist_id: str, playlist_id: str) -> bool:
        ...
        artists_in_playlist = Counter(
            chain.from_iterable(
                [self._extract_artist_ids_for_track(item["track"]) for item in items]
            )
        )
        return (
            artist_id in artists_in_playlist
            and artists_in_playlist[artist_id] >= self.MIN_TRACKS_PER_ARTIST
        )

    def _find_track_uris_from_names(
        self, artist_name: str, track_names: set[str]
    ) -> set[str]:
        track_uris = set()
        ...
        for track in track_names:
            ...
            if len(results["tracks"]["items"]) == 0:
                ...
                if len(track_uris) == self.MAX_TRACKS_PER_ARTIST:
                    logger.info(
                        f"Max tracks for artist ({self.MAX_TRACKS_PER_ARTIST}) reached"
                    )
                    break
    ...
    def _find_all_tracks_by_artist_in_playlist(
        self, artist_id: str, playlist_id: str
    ) -> list[str]:
        items = self.sp.playlist_items(playlist_id)["items"]
        return [
            item["track"]["uri"]
    ...
    def add_artist(self, artist_id: str, playlist_id: str) -> bool:
        ...
        self.sp.playlist_add_items(
            playlist_id=playlist_id, items=[t["uri"] for t in tracks]
        )
        logger.info(
    ...
    def remove_artist(self, artist_id: str, playlist_id: str) -> bool:
        ...
        Returns:
            bool: Indicates whether the playlist was modified in this call.
        """
        tracks_to_remove = self._find_all_tracks_by_artist_in_playlist(
        ...
        if len(tracks_to_remove) > 0:
            ...
            self.sp.playlist_remove_all_occurrences_of_items(
                playlist_id=playlist_id, items=tracks_to_remove
            )
            return True
        else:
    ...
    def add_tracks(
        self, artist_name: str, track_names: set[str], playlist_id: str
    ) -> None:
